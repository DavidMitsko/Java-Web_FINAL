package com.mitsko.mrdb.controller.command.util;

public class Constants {
    public static final String SIGN_IN = "pages/sign_in.jsp";
    public static final String REGISTRATION = "pages/registration.jsp";
    public static final String ADMIN = "pages/take_users.jsp";
    public static final String MAIN = "pages/take_movies.jsp";
    public static final String REVIEW = "pages/take_reviews.jsp";
    public static final String BAN_HTML = "pages/error/ban.html";
    public static final String INDEX = "index.jsp";
    public static final String RATING = "pages/rating.jsp";
    public static final String ERROR = "pages/error.jsp";
    public static final String HISTORY = "pages/user_history.jsp";
    public static final String REMOVE = "pages/take_movies_for_remove.jsp";
    public static final String ADD_MOVIE = "pages/add_movie_page.jsp";

    public static final String TAKE_MOVIE = "take_movies";
    public static final String TAKE_MOVIES_FOR_REMOVE = "take_movies_for_remove";
    public static final String TAKE_USERS = "take_users";

    public static final String MAKE_REDIRECT = "redirect";
}